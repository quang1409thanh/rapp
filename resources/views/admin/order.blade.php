    ...
    <div class="main-panel">
        <div class="content-wrapper">
            ...
            <div class="filter">
                ...
                <div class="sort_by">
                    <form action="{{ url('search') }}" method="get">
                        <label for="sort_by"></label><select style="color: black" name="sort_by" id="sort_by">
                            <option value="" selected="selected">Sort By</option>
                            <option value="name_asc" @if( isset($sortBy)&&$sortBy === 'name_asc') selected @endif>
                                Name (Ascending)
                            </option>
                            <option value="name_desc" @if( isset($sortBy)&&$sortBy === 'name_desc') selected @endif>
                                Name (Descending)
                            </option>
                            <option value="price_asc" @if( isset($sortBy)&&$sortBy === 'price_asc') selected @endif>
                                Price (Ascending)
                            </option>
                            <option value="price_desc" @if( isset($sortBy)&&$sortBy === 'price_desc') selected @endif>
                                Price (Descending)
                            </option>
                            <option value="delivered" @if( isset($sortBy)&&$sortBy === 'delivered') selected @endif>
                                Trạng Thái(Đã xử lý)
                            </option>
                            <option value="processing" @if( isset($sortBy)&&$sortBy === 'processing') selected @endif>
                        ...
                        <td>
                            @if($order->delivery_status == 'Đang Xử Lý')
                                <a href="{{url('delivered', $order->id)}} "
                                   onclick="return confirm('Đơn hàng này đã giao ư ? ')" class="btn btn-primary">
                                    ☑️Delivered</a>
                            @else
